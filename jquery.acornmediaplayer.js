		...
		var acornPlayer = function() {
			...
			*/
			...
			/*
                         ...
                         * If the element is not <video> we leave if blank, as the button if useless on <audio> elements
                         */
                        var downloadBtnMarkup = (acorn.$self.is('video')) ? '<button class="acorn-download-button" title="' + text.downloadTitle + '" aria-controls="' + acorn.id + '" tabindex="4" >' + text.swap + '</button>' : '';
			...
			acorn.$downloadBtn = $('.acorn-download-button', acorn.$container);
			/*
			...
			var goDownload= function(){
				window.location="/download/presentation/"+meetingId;
			}
			...
			var init = function() {
				...
				// bind Swap Button
                                acorn.$downloadBtn.click(goDownload);
